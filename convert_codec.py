...
config.read(config_path)  # Update to the actual location of your properties file
...
output_codec_suffix = config.get('Codecs', 'output_codec_suffix')

# Access variables in the Encoding settings section
speed_preset = config.get('Encoding settings', 'speed_preset')
crf_quality = config.get('Encoding settings', 'crf_quality')
...
completed_files_path = log_and_input_files_path + r"completed_files.txt"  # File to track completed files
error_files_path = log_and_input_files_path +  r"error_files.txt"  # File to track error files
wrong_codec_files_path = log_and_input_files_path +  r"wrong_codec_files.txt"  # File to track files with wrong codec

video_extensions = (".mp4", ".mkv", ".avi", ".mov", ".flv", ".wmv")

# Swap from suffix to technical name 
if (output_codec_suffix.upper() == "H264"):
    output_codec = "libx264"
elif (output_codec_suffix.upper() == "H265"):
    output_codec = "hevc"
else:
    print("Wrong output codec preferences. Quitting program")
    sys.exit()

if (input_codec_suffix.upper() == "H264"):
    input_codec = "libx264"
elif (input_codec_suffix.upper() == "H265"):
    input_codec = "hevc"
else:
    print("Wrong input codec preferences. Quitting program")
    sys.exit()
...
for input_file in input_files:
    ...
    output_file = output_file.replace(".mp4", f" (converted to {output_codec_suffix}).mp4").replace(".mkv", f" (converted to {output_codec_suffix}).mkv")
    ...
    if matching_tuple:
        if (matching_tuple[1] != "copied"):
            success_counter += 1
            print(Fore.CYAN + f"{success_counter+wrong_codec_counter+failed_counter}. File already encoded ({format_seconds_dynamically(int(matching_tuple[1]))}" + Fore.CYAN + f"): {output_file}" + Style.RESET_ALL, sep="")
        else:
    ...
    print(f"\n\n--------------------------------")
    print(f"{success_counter} files successfully encoded")
    print(f"{Fore.RED if failed_counter > 0 else Style.RESET_ALL}{failed_counter} files failed{Style.RESET_ALL}")
    print(f"{Fore.MAGENTA if wrong_codec_counter > 0 else Style.RESET_ALL}{wrong_codec_counter} files not in {input_codec_suffix} {Style.RESET_ALL}", end="")
    if ((copy_files_of_wrong_codec)):
        print("(Copied)")
    else:
        print("")
    print(f"{total_files-success_counter-failed_counter-wrong_codec_counter} files left to encode")
    ...
    try:
        ffprobe_cmd = [
            ...
            "-show_entries", "stream=codec_name", "-of", "default=noprint_wrappers=1:nokey=1", input_file
        ]
    ...
    # If file is "hevc" or "H265", start encoding process
    if codec.lower() == input_codec.lower():
        ...
        # Printout of what file is going to be encoded
        print(
            Fore.GREEN + f"Starting encoding process of a "
        ...
        ffmpeg_cmd = [
            ...
            "-c:v", output_codec, "-crf", crf_quality, "-preset", speed_preset,
        ...
        try:
            ...
            # Stop timer
            encoding_time = int(time.time() - start_time)
            
            # Mark as completed
            with open(completed_files_path, "a") as cf:
                cf.write(f"{encoding_time} {output_file} \n")
            
            # Increment the counter
            success_counter += 1
            total_seconds += encoding_time
            
            print(
                Fore.GREEN + f"\n{output_file}\n"
                + Fore.CYAN + "Successfully finished encoding! \n"
                + Fore.YELLOW + f"File took {format_seconds_dynamically(encoding_time)}{Fore.YELLOW} to encode. "
                + Style.RESET_ALL
            )
              
        # Encoding failed...  
        except subprocess.CalledProcessError:
            print(Fore.RED + f"Error encoding {input_file}. Skipping." + Style.RESET_ALL)
            failed_counter += 1
            
            if not any(input_file in error for error in error_files):
                with open(error_files_path, "a") as cf:
                    cf.write(f"Error while encoding: {input_file}\n")
                error_files.add(f"Error while encoding: {input_file}")
            continue
    else:
        print(Fore.MAGENTA + f"File is not {input_codec_suffix}:\n\n\t{Fore.CYAN}{input_file}" + Style.RESET_ALL)
        wrong_codec_counter += 1
        ...
        if (copy_files_of_wrong_codec):
            ...
            # Mark as completed
            with open(completed_files_path, "a") as cf:
                cf.write(f"copied {output_file} \n")
        
        if not any(input_file in error for error in other_codec_files):
            with open(wrong_codec_files_path, "a") as cf:
                cf.write(f"File is {codec}, not {input_codec}: " + output_file + " \n")
            other_codec_files.add(f"File is {codec}, not {input_codec}: {output_file}")

print(Fore.BLUE + "\n\nSUMMARY:\n" + Style.RESET_ALL)
print(Fore.GREEN + f"Successfully encoded: {success_counter}" + Style.RESET_ALL)
print(Fore.GREEN + f"Wrong codec: {Fore.RED}{wrong_codec_counter}" + Style.RESET_ALL)
print(Fore.GREEN + f"Failed encodings: {Fore.RED}{failed_counter}" + Style.RESET_ALL)
